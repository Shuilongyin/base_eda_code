...
class MDLP_Discretizer(object):
    def __init__(self, dataset, class_label, features=None):
        ...
        self._data_new = self._data_raw.copy(deep = True)
        ...
        self._gains = {f: [] for f in self._features}        
    ...
    def MDLPC_criterion(self, data, feature, cut_point, per=1):
        '''
    ...
    def single_feature_accepted_cutpoints(self, feature, partition_index=pd.DataFrame().index):
        ...
        #print (feature,'cut_candidate:%s'%cut_candidate)
        ...
        #apply decision
        if not decision:
            self._gains[feature].append(cut_candidate)
        ...
        if decision:
            ...
            self.single_feature_accepted_cutpoints(feature=feature, partition_index=left_partition.index) #递归
            self.single_feature_accepted_cutpoints(feature=feature, partition_index=right_partition.index)#递归
    ...
    def apply_cutpoints(self):
        ...
        for f in self._cuts:
            fDict = {}
            f_cutpoints = self._cuts[f]
            if len(f_cutpoints)==0:
                self._notDiscretizer.append(f)
            elif len(f_cutpoints)==1:
                fDict[1] = (self._data_new.loc[(self._data_new[f]<=f_cutpoints[0]),f]).index
                fDict[2] = (self._data_new.loc[(self._data_new[f]> f_cutpoints[0]),f]).index
                self._data_new.ix[fDict[1],f] = 1
                self._data_new.ix[fDict[2],f] = 2
            else:
                fDict[1] = (self._data_new.loc[(self._data_new[f]<=f_cutpoints[0]),f]).index
                fDict[len(f_cutpoints)+1] = (self._data_new.loc[(self._data_new[f]> f_cutpoints[len(f_cutpoints)-1]),f]).index
                for i in range(len(f_cutpoints)-1):
                    fDict[i+2] = (self._data_new.loc[((self._data_new[f]>f_cutpoints[i]) & (self._data_new[f]<=f_cutpoints[i+1])),f]).index
                for k in fDict.keys():
                    self._data_new.ix[fDict[k],f] = k
#            if self._data_new[f].isnull().values.any and len(f_cutpoints)!=0:
#                self._data_new[f].fillna(value=0,inplace=True)      
...
if __name__ == '__main__':
    ...
    mydat = DataFrame(X,columns=['a','b','c','d']).join(DataFrame(y,columns=['y']))
    mdlp = MDLP_Discretizer(mydat,'y',['a','b','c','d'])
